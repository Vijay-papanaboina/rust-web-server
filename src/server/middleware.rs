...
impl Middleware {
    ...
    pub async fn check_auth(&self, request: &mut Request, stream: &mut TcpStream) -> Result<(), Box<dyn Error>> {
        let auth_header = match request.headers.get("Authorization") {
            Some(header) => header,
            None => {
                let response = r#"{"error": "Authorization header is missing"}"#;
                let _ = send_response(stream, StatusCode::Unauthorized, "application/json", response.as_bytes()).await;
